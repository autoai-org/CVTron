...
class Classification(object):
        ...
        for shard_id in range(num_shards):
          ...
          with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
            ...
            for i in range(start_ndx, end_ndx):
              tf.logging.info('>> Converting image %d/%d shard %d' % (i + 1), len(photo_class), shard_id)
  ...
  def get_filenames_and_classes(self):
    photo_class_names = []
    class_names = []
    ...
    with open(annotation_file) as f:
      ...
      for i in range(img_num):
        path = os.path.join(annotation_data[i]['folder'], annotation_data[i]['filename'])
  ...
  def create_tf_data(self, ratio=0.7):
    photo_class, class_names = self.get_filenames_and_classes()
    ...
    dataset_utils.write_label_file(ids_to_class_names, self.training_configs['dataset_params']['dataset_dir'] + 'label_map.txt')
