    ...
    CVTron has some prerequisites that need to be installed first, including
    ...
    Mac OX, Linux and Windows, or ask for help on `CVTron@gmail.com <CVTron@gmail.com>`_
    or `FQA <http://CVTron.readthedocs.io/en/latest/user/more.html>`_.
    ...
    CVTron is build on the top of Python-version TensorFlow, so please install
    Python first.
    ...
    (You can also skip to Step 3, automatically install the prerequisites by CVTron)
    ...
    [stable version] pip install CVTron
    [master version] pip install git+https://github.com/zsdonghao/CVTron.git

    However, if you want to modify or extend CVTron, you can download the repository from
    ...
    This command will run the ``setup.py`` to install CVTron. The ``-e`` reflects
    editable, then you can edit the source code in ``CVTron`` folder, and ``import`` the edited
    CVTron.
    ...
    .. _GitHub: https://github.com/zsdonghao/CVTron
    .. _CVTron: https://github.com/zsdonghao/CVTron/
    ...
    CVTron is built on the top of Python-version TensorFlow, so please install Python first.
    ...
    pip install CVTron       #Install CVTron
    ...
    If you get the following output when import CVTron, please read `FQA <http://CVTron.readthedocs.io/en/latest/user/more.html>`_.
